...
#define adc_12v_min 2800
#define adc_12v_max 3500
#define adc_9v_min 900
#define adc_9v_max 3000
#define adc_6v_min 400
#define adc_6v_max 850
    ...
    if (   digitalRead(start_pin) == HIGH)
    {
      ...
      if (  digitalRead(start_pin) == HIGH)
      {
        ...
        if (  digitalRead(start_pin) == LOW) {
          ...
          lcd.print("Starting...");
          _delay_ms(2000);
          Serial.println("buttonpresss instance");
          start_flag = true;
          ...
          if (duty_flag == 0) {
            duty_flag = 1;
            Serial.println("Changed to 50% duty cycle");
            lcd.clear();
            lcd.setCursor(0, 0);
            lcd.print("Mode Changed to 50%");
          }
          else if (duty_flag == 1) {
            duty_flag = 0;
            Serial.println("Changed to 25% duty cycle");
            lcd.clear();
            lcd.setCursor(0, 0);
            lcd.print("Mode Changed to 25%");
          }
        }
      }
...
void setup() {
 ...
 // emon1.current(current_ip_pin, 17);
  emon1.current(current_ip_pin, 4.5); // For 5111
  ...
  if (start_flag == true)
  {
    ...
    if (voltage > 100) {
      ...
      else if ((adc_9v_min < voltage) && (voltage < adc_9v_max)) //9v
      {
        ...
        {
          if(duty_flag==0){            //decide mode
          analogWrite(pwm_1k, duty_cycle);
          }
          else if(duty_flag==1){
          analogWrite(pwm_1k, duty_cycle1);
          }
          digitalWrite(relay_live, LOW);
          digitalWrite(relay_neutral, LOW);
