...
void start_detect()                          //Switch ON button
{
  if (start_flag == false)
  {
    if (   digitalRead(start_pin) == HIGH || user_input=="STRT")
    {
      _delay_ms(50);
      if (  digitalRead(start_pin) == HIGH || user_input=="STRT")
      {
...
void setup() {
  ...
  Serial.println();
  Serial.println();
  Serial.println("ENTER -STRT- to START and  -STOP- to stop the code");
  Serial.println();
  Serial.println();
...

{
   ...
   if(Serial.available()){
        ...
        Serial.println("Entered command : " + user_input );

        if(user_input=="STOP"){stop_detect();}
    }
  ...
  if (start_flag == true )  
  {
    ...
    for (int i = 0; i < 100; i++)
    {
      ...
      if (volt9_flag == 1)
      {
        if (voltage > 100)       //analog read square wave problem 
        {
          voltagetotal += voltage;
          counterval++;
        }
      }
      else if (volt6_flag == 1)   //analog read square wave problem
      {
        //        if (voltage > 420)
        //        {
        //          stop_now();
      ...
      else       //analog read square wave problem
      {
        if (voltage > 50)
        {
          voltagetotal += voltage;
